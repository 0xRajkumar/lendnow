...
contract Pool {
    ...
    function lend(
        ...
        uint256 totalAmount = tokenData.totalCollateral.amount +
            tokenData.totalBorrow.amount;
        uint256 inShare = toShares(
            tokenData.totalCollateral.shares,
            totalAmount,
            amount
        );
        tokenData.totalCollateral.shares =
        ...
        if (token == token0) {
            userData.token0CollateralShare =
                userData.token0CollateralShare +
                uint128(inShare);
        } else {
    ...
    function redeem(
        ...
        if (token == token0) {
            require(
                userData.token0CollateralShare >= shareAmount,
                "Low collateral Balance"
            );
        } else {
            require(
                userData.token1CollateralShare >= shareAmount,
                "Low collateral Balance"
            );
        }
        TokenData storage tokenData = (token == token0)
    ...
    function repay(
        ...
        if (token == token0) {
            require(
                userData.token0BorrowShare >= shareAmount,
                "Invalid share amount"
            );
        } else {
            require(
                userData.token1BorrowShare >= shareAmount,
                "Invalid share amount"
            );
        }
        TokenData storage tokenData = (token == token0)
    ...
    function liquidate(
        ...
        if (token0 == debtAsset) {
            ...
            uint256 minusFromUser = toShares(
                token1Data.totalCollateral.shares,
                token1Data.totalCollateral.amount,
                token1Amount
            );
            ...
            uint256 minusFromUser = toShares(
                token0Data.totalCollateral.shares,
                token0Data.totalCollateral.amount,
                token0Amount
            );
    ...
    function accrueInterest() public {
        ...
        if (remainingBlocks > 0) {
            ...
            token0Data.totalBorrow.amount += uint128(
                calculateInterest(
                    token0Data.totalBorrow.amount,
                    interestRateForToken0,
                    remainingBlocks
                )
            );
            ...
            token1Data.totalBorrow.amount += uint128(
                calculateInterest(
                    token1Data.totalBorrow.amount,
                    interestRateForToken1,
                    remainingBlocks
                )
            );
    ...
    function toShares(
        ...
        if (totalAmount == 0) {
            ...
            shares = (totalShares * amount) / totalAmount;
        }
    }
    ...
    function getUserCollateralShares(
        address user
    ...
    function getUserTotalCollateral(
        ...
        uint256 totalToken1Amount = token1Data.totalCollateral.amount +
            token1Data.totalBorrow.amount;
        uint256 token1Amount = toAmount(
            totalToken1Amount,
            token1Data.totalCollateral.shares,
            userData.token1CollateralShare
        );
    ...
    function getHealthFactor(address user) public view returns (uint256) {
        ...
        return
            (((userTotalCollateral * LIQUIDATION_THRESHOLD) / 100) * 1e18) /
            usertotalBorrow;
    }
}
